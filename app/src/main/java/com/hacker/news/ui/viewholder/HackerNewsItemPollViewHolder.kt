...
import kotlinx.android.synthetic.main.list_item_hacker_news_poll.view.*
import java.text.NumberFormat
import java.util.*

class HackerNewsItemPollViewHolder private constructor(
    itemView: View,
    ...
    companion object {
        fun create(
            ...
            HackerNewsItemPollViewHolder(
                LayoutInflater.from(parent.context).inflate(R.layout.list_item_hacker_news_poll, parent, false),
                onClickAction
            )
    }
    ...
    init {
        ...
        itemView.showCommentsButton.setOnClickListener {
            onClickAction(adapterPosition, HackerNewsItemAdapter.ClickActionType.SHOW_COMMENTS)
        }

        itemView.subTitleTextView.setOnClickListener {
            onClickAction(adapterPosition, HackerNewsItemAdapter.ClickActionType.SHOW_PROFILE)
        }

        itemView.showPollOptionsButton.setOnClickListener {
            onClickAction(adapterPosition, HackerNewsItemAdapter.ClickActionType.SHOW_POLLS)
        }
    ...
    override fun bindView(model: ItemInfoResponse) {
        ...
        if (model.kids.isNullOrEmpty()) {
            itemView.showCommentsButton.text =
                itemView.context.resources.getQuantityString(R.plurals.comments, 0, 0)
            itemView.showCommentsButton.isEnabled = false
        } else {
            itemView.showCommentsButton.text =
                itemView.context.resources.getQuantityString(R.plurals.comments, model.kids!!.size, model.kids!!.size)
            itemView.showCommentsButton.isEnabled = true
        }

        itemView.showPollOptionsButton.isVisible = !model.parts.isNullOrEmpty()
    }
